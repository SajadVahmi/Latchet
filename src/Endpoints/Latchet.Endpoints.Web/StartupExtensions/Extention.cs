using Latchet.Utilities.Services.DependentyInjection;
...
namespace Latchet.Endpoints.Web.StartupExtensions
{
    public static class Extentions
    {
        ...
        public static IServiceCollection AddLatchetDependencies(this IServiceCollection services,
            params string[] assemblyNamesForSearch)
        {

            var assemblies = GetAssemblies(assemblyNamesForSearch);
            services.AddApplicationServices(assemblies).AddDataAccess(assemblies).AddLatchetServices(assemblies).AddCustomeDepenecies(assemblies);
            return services;
        }
